...
let private fsharp31Portable78AssembliesPath = 
    referenceAssembliesPath
    ++ "FSharp" 
    ++ ".NETCore" 
    ++ "3.78.3.1" 
...
let private getAssembly (asmName:AssemblyName) reflectionOnly fsharpDataPaths = 
    let folders = 
        ...
        | "FSharp.Core", "3.78.3.1" -> [fsharp31Portable78AssembliesPath]
...
type FSharpDataRuntimeVersion =
    ...
    | Portable78 //net45+wpsl8+win8
    | Portable259 //net45+wpsl8+win8+wp81
...
let init (cfg : TypeProviderConfig) = 
    ...
    let runtimeVersion =
        ...
        | 78 -> FSharpDataRuntimeVersion.Portable78
        | 259 -> FSharpDataRuntimeVersion.Portable259
        | _ -> failwithf "Unexpected version of %s.dll: %O [Looking for revision 0, 7, 47, 78 or 259]" runtimeAssemblyName.Name runtimeAssemblyName.Version
